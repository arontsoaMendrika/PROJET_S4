...
(function() {
    ...
    if (signup) {
        ...
        signup.addEventListener('submit', function(e) {
            ...
            if (!password || password.length < 8 || !/[A-Z]/.test(password) || !/\d/.test(password)) {
                showError('password', 'Mot de passe: min 8 caractères, 1 majuscule et 1 chiffre.');
                ok = false;
            }
            if (!conditions) {
    ...
    if (health) {
        // calc button handler
        if (!naissance) { showError('naissance', 'La date de naissance est requise.');
            ok = false; }
        const calc = qs('calc-imc');
        if (!telephone) { showError('telephone', 'Telephone requis.');
            ok = false; }
        if (!pays) { showError('pays', 'Veuillez choisir un pays.');
            ok = false; }
        ...
        health.addEventListener('submit', function(e) {
            ...
            if (!t || !p || t <= 0 || p <= 0) {
                e.preventDefault();
                out.textContent = 'Taille et poids valides requis.';
                return;
            }
