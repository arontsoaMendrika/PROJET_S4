    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm rounded-0 p-4" style="background:#fff;">
                <form action="<?= base_url('regime/store') ?>" method="POST">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-3 mb-md-0">
                            ...
                            <input type="text" name="name" class="form-control rounded-0 border-dark" required placeholder="Ex: Keto Premium" value="<?= esc($_SESSION['old']['name'] ?? '') ?>">
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="regime_type_id" class="form-select rounded-0 border-dark" required>
                                ...
                                <?php if(isset($types) && is_array($types)): ?>
                                    <?php foreach ($types as $type): ?>
                                        <option value="<?= esc($type['id']) ?>" <?= (isset($_SESSION['old']['regime_type_id']) && $_SESSION['old']['regime_type_id'] == $type['id']) ? 'selected' : '' ?>><?= esc($type['name']) ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" class="form-control rounded-0 border-dark" rows="4" required placeholder="Description détaillée du programme..."><?= esc($_SESSION['old']['description'] ?? '') ?></textarea>
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-3 mb-md-0">
                            ...
                            <select name="goal" class="form-select rounded-0 border-dark" required>
                                <option value="weight_loss" <?= (isset($_SESSION['old']['goal']) && $_SESSION['old']['goal'] == 'weight_loss') ? 'selected' : '' ?>>Perte de poids</option>
                                <option value="maintenance" <?= (isset($_SESSION['old']['goal']) && $_SESSION['old']['goal'] == 'maintenance') ? 'selected' : '' ?>>Maintien</option>
                                <option value="muscle_gain" <?= (isset($_SESSION['old']['goal']) && $_SESSION['old']['goal'] == 'muscle_gain') ? 'selected' : '' ?>>Prise de masse</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small text-muted text-uppercase">Durée recommandée (Jours)</label>
                            <input type="number" name="duration_days" class="form-control rounded-0 border-dark" value="<?= esc($_SESSION['old']['duration_days'] ?? '30') ?>">
                        </div>
                    </div>
                    ...
                    <div class="row mb-5">
                        <div class="col-md-6">
                            ...
                            <div class="input-group rounded-0">
                                <input type="number" step="0.01" name="price_per_day" class="form-control rounded-0 border-dark border-end-0" required value="<?= esc($_SESSION['old']['price_per_day'] ?? '') ?>">
                    ...
                    <div class="d-flex gap-3">
                        <a href="<?= base_url('regimes') ?>" class="btn btn-outline-dark rounded-0 py-2 px-4 text-uppercase" style="letter-spacing: 1px; font-size: 0.85rem;">Annuler</a>
