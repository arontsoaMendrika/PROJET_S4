    ...
    <div class="row justify-content-center">
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm rounded-0 p-4" style="background:#fff;">
                <form action="<?= base_url('regimes/update/' . $regime['id']) ?>" method="POST">
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-3 mb-md-0">
                            ...
                            <input type="text" name="name" class="form-control rounded-0 border-dark" required
                                value="<?= esc($_SESSION['old']['name'] ?? $regime['name']) ?>">
                        </div>
                        <div class="col-md-6">
                            ...
                            <select name="regime_type_id" class="form-select rounded-0 border-dark" required>
                                <?php if (isset($types) && is_array($types)): ?>
                                    <?php foreach ($types as $type): ?>
                                        <option value="<?= esc($type['id']) ?>" <?= ((isset($_SESSION['old']['regime_type_id']) && $_SESSION['old']['regime_type_id'] == $type['id']) || (!isset($_SESSION['old']['regime_type_id']) && $regime['regime_type_id'] == $type['id'])) ? 'selected' : '' ?>>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" class="form-control rounded-0 border-dark" rows="4"
                            required><?= esc($_SESSION['old']['description'] ?? $regime['description']) ?></textarea>
                    </div>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6 mb-3 mb-md-0">
                            ...
                            <input type="number" name="calories_per_day" class="form-control rounded-0 border-dark"
                                required
                                value="<?= esc($_SESSION['old']['calories_per_day'] ?? $regime['calories_per_day'] ?? '') ?>">
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="number" name="duration_days" class="form-control rounded-0 border-dark"
                                required
                                value="<?= esc($_SESSION['old']['duration_days'] ?? $regime['duration_days'] ?? '') ?>">
                        </div>
                    </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="input-group rounded-0">
                                <input type="number" step="0.01" name="base_price"
                                    ...
                                    value="<?= esc($_SESSION['old']['base_price'] ?? $regime['base_price'] ?? '') ?>">
                                <span
